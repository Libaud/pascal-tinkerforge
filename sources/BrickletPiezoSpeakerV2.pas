...
type
  ...
  public
    ...
    /// </summary>
    procedure SetBeep(const frequency: word; const volume: byte; const duration: longword); virtual;
    ...
    /// </summary>
    procedure GetBeep(out frequency: word; out volume: byte; out duration: longword; out durationRemaining: longword); virtual;
    ...
    /// </summary>
    procedure SetAlarm(const startFrequency: word; const endFrequency: word; const stepSize: word; const stepDelay: word; const volume: byte; const duration: longword); virtual;
    ...
    /// </summary>
    procedure GetAlarm(out startFrequency: word; out endFrequency: word; out stepSize: word; out stepDelay: word; out volume: byte; out duration: longword; out durationRemaining: longword; out currentFrequency: word); virtual;

    /// <summary>
    ///  Updates the volume of an ongoing beep or alarm. The range of the volume is 0 to 10.
    /// </summary>
    procedure UpdateVolume(const volume: byte); virtual;

    /// <summary>
    ///  Updates the frequency of an ongoing beep. The range of the frequency is 50Hz to 15000Hz.
    /// </summary>
    procedure UpdateFrequency(const frequency: word); virtual;
...
procedure TBrickletPiezoSpeakerV2.SetBeep(const frequency: word; const volume: byte; const duration: longword);
var 
_request: TDynamicByteArray;
begin
  _request:= IPConnection.CreateRequestPacket(self, BRICKLET_PIEZO_SPEAKER_V2_FUNCTION_SET_BEEP, 15);
  LEConvertUInt16To(frequency, 8, _request);
  LEConvertUInt8To(volume, 10, _request);
  LEConvertUInt32To(duration, 11, _request);
  SendRequest(_request);
end;

procedure TBrickletPiezoSpeakerV2.GetBeep(out frequency: word; out volume: byte; out duration: longword; out durationRemaining: longword);
var 
_request, _response: TDynamicByteArray;
begin
  _request:= IPConnection.CreateRequestPacket(self, BRICKLET_PIEZO_SPEAKER_V2_FUNCTION_GET_BEEP, 8);
  _response:= SendRequest(_request);
  frequency:= LEConvertUInt16From(8, _response);
  volume:= LEConvertUInt8From(10, _response);
  duration:= LEConvertUInt32From(11, _response);
  durationRemaining:= LEConvertUInt32From(15, _response);
end;

procedure TBrickletPiezoSpeakerV2.SetAlarm(const startFrequency: word; const endFrequency: word; const stepSize: word; const stepDelay: word; const volume: byte; const duration: longword);
var 
_request: TDynamicByteArray;
begin
  _request:= IPConnection.CreateRequestPacket(self, BRICKLET_PIEZO_SPEAKER_V2_FUNCTION_SET_ALARM, 21);
  LEConvertUInt16To(startFrequency, 8, _request);
  LEConvertUInt16To(endFrequency, 10, _request);
  LEConvertUInt16To(stepSize, 12, _request);
  LEConvertUInt16To(stepDelay, 14, _request);
  LEConvertUInt8To(volume, 16, _request);
  LEConvertUInt32To(duration, 17, _request);
  SendRequest(_request);
end;

procedure TBrickletPiezoSpeakerV2.GetAlarm(out startFrequency: word; out endFrequency: word; out stepSize: word; out stepDelay: word; out volume: byte; out duration: longword; out durationRemaining: longword; out currentFrequency: word);
var 
_request, _response: TDynamicByteArray;
begin
  _request:= IPConnection.CreateRequestPacket(self, BRICKLET_PIEZO_SPEAKER_V2_FUNCTION_GET_ALARM, 8);
  _response:= SendRequest(_request);
  startFrequency:= LEConvertUInt16From(8, _response);
  endFrequency:= LEConvertUInt16From(10, _response);
  stepSize:= LEConvertUInt16From(12, _response);
  stepDelay:= LEConvertUInt16From(14, _response);
  volume:= LEConvertUInt8From(16, _response);
  duration:= LEConvertUInt32From(17, _response);
  durationRemaining:= LEConvertUInt32From(21, _response);
  currentFrequency:= LEConvertUInt16From(25, _response);
end;

procedure TBrickletPiezoSpeakerV2.UpdateVolume(const volume: byte);
var 
_request: TDynamicByteArray;
begin
  _request:= IPConnection.CreateRequestPacket(self, BRICKLET_PIEZO_SPEAKER_V2_FUNCTION_UPDATE_VOLUME, 9);
  LEConvertUInt8To(volume, 8, _request);
  SendRequest(_request);
end;

procedure TBrickletPiezoSpeakerV2.UpdateFrequency(const frequency: word);
var 
_request: TDynamicByteArray;
begin
...
procedure TBrickletPiezoSpeakerV2.GetSPITFPErrorCount(out errorCountAckChecksum: longword; out errorCountMessageChecksum: longword; out errorCountFrame: longword; out errorCountOverflow: longword);
var 
_request, _response: TDynamicByteArray;
begin
...
procedure TBrickletPiezoSpeakerV2.SetWriteFirmwarePointer(const pointer: longword);
var 
_request: TDynamicByteArray;
begin
...
function TBrickletPiezoSpeakerV2.WriteFirmware(const data: array of byte): byte;
var 
...
procedure TBrickletPiezoSpeakerV2.SetStatusLEDConfig(const config: byte);
var 
_request: TDynamicByteArray;
begin
...
procedure TBrickletPiezoSpeakerV2.GetIdentity(out aUID: string; out connectedUid: string; out position: char; out hardwareVersion: TTFVersionNumber; out firmwareVersion: TTFVersionNumber; out deviceIdentifier: word);
var 
_request, _response: TDynamicByteArray; _i: longint;
begin
