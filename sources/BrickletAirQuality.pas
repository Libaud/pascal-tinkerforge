...
type
  ...
  TBrickletAirQualityNotifyAllValues = procedure(aSender: TBrickletAirQuality; const iaqIndex: longint; const iaqIndexAccuracy: byte; const temperature: longint; const humidity: longint; const airPressure: longint) of object;
  TBrickletAirQualityNotifyIAQIndex = procedure(aSender: TBrickletAirQuality; const iaqIndex: longint; const iaqIndexAccuracy: byte) of object;
  TBrickletAirQualityNotifyTemperature = procedure(aSender: TBrickletAirQuality; const temperature: longint) of object;
  TBrickletAirQualityNotifyHumidity = procedure(aSender: TBrickletAirQuality; const humidity: longint) of object;
  TBrickletAirQualityNotifyAirPressure = procedure(aSender: TBrickletAirQuality; const airPressure: longint) of object;
  ...
  public
    ...
    /// </summary>
    procedure GetAllValues(out iaqIndex: longint; out iaqIndexAccuracy: byte; out temperature: longint; out humidity: longint; out airPressure: longint); virtual;
    ...
    ///  the measurements.
    /// </summary>
    procedure SetTemperatureOffset(const offset: longint); virtual;
    ...
    /// </summary>
    procedure GetIAQIndex(out iaqIndex: longint; out iaqIndexAccuracy: byte); virtual;
    ...
    ///  independent of the value.
    /// </summary>
    procedure SetIAQIndexCallbackConfiguration(const period: longword; const valueHasToChange: boolean); virtual;

    /// <summary>
    ///  Returns the callback configuration as set by
    ///  <see cref="BrickletAirQuality.TBrickletAirQuality.SetIAQIndexCallbackConfiguration"/>.
    /// </summary>
    procedure GetIAQIndexCallbackConfiguration(out period: longword; out valueHasToChange: boolean); virtual;
    ...
    ///  If the option is set to 'x' (threshold turned off) the callback is triggered with the fixed period.
    /// </summary>
    procedure SetHumidityCallbackConfiguration(const period: longword; const valueHasToChange: boolean; const option: char; const min: longint; const max: longint); virtual;
    ...
    /// </summary>
    procedure GetHumidityCallbackConfiguration(out period: longword; out valueHasToChange: boolean; out option: char; out min: longint; out max: longint); virtual;
    ...
    ///  .. versionadded:: 2.0.3$nbsp;(Plugin)
    /// </summary>
    procedure SetBackgroundCalibrationDuration(const duration: byte); virtual;
...
begin
  ...
  iaqIndex:= LEConvertInt32From(8, _response);
  iaqIndexAccuracy:= LEConvertUInt8From(12, _response);
  temperature:= LEConvertInt32From(13, _response);
  humidity:= LEConvertInt32From(17, _response);
  airPressure:= LEConvertInt32From(21, _response);
end;
...
begin
  ...
  LEConvertInt32To(offset, 8, _request);
...
begin
  ...
  LEConvertUInt8To(duration, 8, _request);
  SendRequest(_request);
end;
...
function TBrickletAirQuality.WriteFirmware(const data: array of byte): byte;
var 
...
procedure TBrickletAirQuality.GetIdentity(out aUID: string; out connectedUid: string; out position: char; out hardwareVersion: TTFVersionNumber; out firmwareVersion: TTFVersionNumber; out deviceIdentifier: word);
var 
_request, _response: TDynamicByteArray; _i: longint;
begin
...
var iaqIndex: longint; iaqIndexAccuracy: byte; temperature: longint; humidity: longint; airPressure: longint;
begin
  iaqIndex:= LEConvertInt32From(8, aPacket);
  iaqIndexAccuracy:= LEConvertUInt8From(12, aPacket);
  temperature:= LEConvertInt32From(13, aPacket);
  humidity:= LEConvertInt32From(17, aPacket);
  airPressure:= LEConvertInt32From(21, aPacket);

  if (Assigned(fAllValuesCallback)) then begin
    fAllValuesCallback(self, iaqIndex, iaqIndexAccuracy, temperature, humidity, airPressure);
  end;
end;

procedure TBrickletAirQuality.CallbackWrapperIAQIndex(const aPacket: TDynamicByteArray);
var iaqIndex: longint; iaqIndexAccuracy: byte;
begin
  iaqIndex:= LEConvertInt32From(8, aPacket);
  iaqIndexAccuracy:= LEConvertUInt8From(12, aPacket);

  if (Assigned(fIaqIndexCallback)) then begin
    fIaqIndexCallback(self, iaqIndex, iaqIndexAccuracy);
  end;
end;
...
var humidity: longint;
begin
  humidity:= LEConvertInt32From(8, aPacket);
